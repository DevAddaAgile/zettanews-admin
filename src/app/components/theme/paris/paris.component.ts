...
@Component({
    ...
    imports: [PageWrapperComponent, ReactiveFormsModule, NgbNavModule,
        FormFieldsComponent, ImageUploadComponent, LinkComponent, NgbNavOutlet,
        ButtonComponent, NgbAccordionModule, Select2Module,
        HasPermissionDirective, CommonModule, TranslateModule]
})
export class ParisComponent {

  product$: Observable<Select2Data> = inject(Store).select(ProductState.products);
  home_page$: Observable<Paris> = inject(Store).select(ThemeState.homePage);
